			...
			public function __construct()
			{
				...
				switch ($act) {
					...
					case "do_edit":
							...
							$this->model->execute("update tbl_product set fk_category_product_id=$fk_category_product_id,c_name='$c_name',c_hotproduct=$c_hotproduct,c_description='$c_description',c_price=$c_price,c_content='$c_content' where pk_product_id=$id");
					...
					case "do_add":
							...
							$c_price = $_POST["c_price"];
							$fk_category_product_id =$_POST["fk_category_product_id"];
						//kiem tra, neu user co upload anh thi thuc hien insert
						if($_FILES["c_img"]["name"] != ""){
						$c_img = time().$_FILES["c_img"]["name"];
						//upload anh
						move_uploaded_file($_FILES["c_img"]["tmp_name"], "public/upload/product/$c_img");
						//insert ban ghi vao csdl
						$this->model->execute("insert into tbl_product(fk_category_product_id,c_name,c_description,c_content,c_img,c_price,c_hotproduct)
							values($fk_category_product_id,'$c_name','$c_description','$c_content','$c_img',$c_price,$c_hotproduct) ");
					}
					header("location:admin.php?controller=product");
				break;
